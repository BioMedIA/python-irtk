...
def voxellise( points, triangles, header=None, pixelSize=[1,1,1,1] ):
    points = np.array( points, dtype='float64', order='C' )
    triangles = np.array( triangles, dtype='int32', order='C' )
...
def read_polydata( filename ):
    return _irtk.read_polydata(filename)
...
def shrinkSphere( points, header ):
    img = _irtk.shrinkSphere( points, header )
    return image.Image( img, header )
