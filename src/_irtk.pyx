...
def gdt( np.ndarray[float, ndim=4,  mode="c"] img_in,
         np.ndarray[unsigned char, ndim=4,  mode="c"] mask,
         header,
         float l ):
    cdef np.ndarray[double, ndim=1,  mode="c"] pixelSize = header['pixelSize']
    cdef np.ndarray[double, ndim=1,  mode="c"] xAxis = header['orientation'][0]
...
def anisoDiff( np.ndarray[float, ndim=4,  mode="c"] img_in,
               header,
               int iterations ):
    cdef np.ndarray[double, ndim=1,  mode="c"] pixelSize = header['pixelSize']
    cdef np.ndarray[double, ndim=1,  mode="c"] xAxis = header['orientation'][0]
...
def read_affine( bytes py_string ):
    cdef char* c_string = py_string
    cdef double tx, ty, tz, rx, ry, rz, sx, sy, sz, sxy, syz, sxz
    _read_affine( c_string,
                  tx, ty, tz,
                  rx, ry, rz,
                  sx, sy, sz,
                  sxy, syz, sxz )
...
def write_affine( bytes py_string,
                  double tx,
                  double ty,
                  double tz,
                  double rx,
...
def affine_matrix( double tx,
                   ...
                   double syz,
                   double sxz ):
    cdef np.ndarray[double, ndim=2,  mode="c"] m = np.zeros( (4,4),
                                                             dtype="float64" )

    _affine_matrix( tx, ty, tz,
                    rx, ry, rz,
                    sx, sy, sz,
                    sxy, syz, sxz,
                    <double*>m.data )
...
def affine_from_matrix( np.ndarray[double, ndim=2,  mode="c"] m ):
    cdef double tx, ty, tz, rx, ry, rz, sx, sy, sz, sxy, syz, sxz 
    _affine_from_matrix( tx, ty, tz,
                         rx, ry, rz,
                         sx, sy, sz,
                         sxy, syz, sxz,
                         <double*> m.data )
...
def transform_affine( double tx,
                      double ty,
                      double tz,
                      double rx,
                      double ry,
...
def shrinkSphere( np.ndarray[double, ndim=2,  mode="c"] points,
                  header ):
    cdef np.ndarray[double, ndim=1,  mode="c"] pixelSize = header['pixelSize']
    cdef np.ndarray[double, ndim=1,  mode="c"] xAxis = header['orientation'][0]
    cdef np.ndarray[double, ndim=1,  mode="c"] yAxis = header['orientation'][1]
    ...
    _shrinkSphere( <double*> points.data,
                    points.shape[0],
                    <uchar*> img.data,
                    <double*> pixelSize.data,
                    <double*> xAxis.data,
...
def read_polydata( bytes filename ):
    cdef char* c_string = filename
    cdef vector[vector[double]] points
    cdef vector[vector[int]] triangles

    _read_polydata( c_string,
                    points,
                    triangles )
...
def registration_affine_points( np.ndarray[double, ndim=2,  mode="c"] source,
                               np.ndarray[double, ndim=2,  mode="c"] target ):
    cdef double tx
    cdef double ty
    cdef double tz
    ...
    cdef double syz = -1
    cdef double sxz = -1
    cdef double RMS = _registration_affine_points( <double*> source.data,
                                                    <double*> target.data,
                                                    source.shape[0],
                                                    tx, ty, tz,
                                                    rx, ry,rz,
...
def crf( np.ndarray[pixel_t, ndim=4, mode="c"] img,
    ...
    cdef np.ndarray[double, ndim=1,  mode="c"] origin = header['origin']
    cdef np.ndarray[int, ndim=1,  mode="c"] dim =  header['dim']
    cdef int nb_labels = proba.shape[0]
    print "starting crf..."                                                                 
...
def graphcut( np.ndarray[pixel_t, ndim=4, mode="c"] img,
              header,
              np.ndarray[LabelID, ndim=4, mode="c"] labels,
              double l=1.0,
              double sigma=0.0,
