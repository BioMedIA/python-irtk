...
int _get_header( char* filename,
    ...
    try {
        ...
        irtkImageAttributes attr = reader->GetImageAttributes();
        get_attributes( attr,
                    pixelSize,
                    xAxis,
                    yAxis,
...
void _gdt( float* img_in,
    ...
    irtkGenericImage<unsigned char> irtk_mask;
    py2irtk<unsigned char>( irtk_mask,
                            mask,
                            pixelSize,
                            xAxis,
                            yAxis,
    ...
    irtkGenericImage<float> distanceMap;
    irtkGDT gdt_filter(irtk_image,irtk_mask,distanceMap);
    gdt_filter.SetLambda(lambda);
    gdt_filter.Run();

    //gdt_filter.GetOutput(irtk_image);

    irtk2py<float>( distanceMap,
                    img_out,
                    pixelSize,
                    xAxis,
                    yAxis,
...
void _anisoDiff( float* img_in,
    ...
    anisoDiffusion<float> anisodiff;
    anisodiff.SetInput(&irtk_image);
    anisodiff.SetOutput(&irtk_image);
    anisodiff.ITERATIONS_NB = iterations;
    anisodiff.Run();
